...
function App() {
  ...
  const onResults = useCallback((results: any) => {
    ...
    if (landmarks && landmarks.length >= 468) {
      ...
      // For MAPPED gaze (used in post-calibration stages)
  ...
  const handleMorningComplete = () => {
    console.log('Morning stage complete, restarting from intro');
    setStage('intro');
    stageRef.current = 'intro';
    // Increment day number when going from morning back to intro
